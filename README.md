...
The API also exposes a protected artwork probe while thumbnail support is being evaluated:

```bash
curl -o artwork "http://<windows-host>:5002/api/v1/<secret>/artwork"
```

It returns the thumbnail published by the current Windows Media Session, or `404` when no thumbnail is available. This endpoint is intentionally not wired into the Home Assistant entity yet; Chrome and browser-based players may not publish artwork through Windows.
